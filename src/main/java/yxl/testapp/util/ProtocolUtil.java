 ...
 * 自定义协议工具类
 * 4byte    4byte   payload     5byte
 * 数据长度  魔数     protobuf    终止符
 */
...
public class ProtocolUtil {
    ...
    public byte[] decodeProtocol(byte[] data) {
        ...
        byte[] end = new byte[5];
        System.arraycopy(data, 4 + 4 + len, end, 0, 5);
        if (!FinalData.END.equals(new String(end))) {
            return null;
        }
        byte[] result = new byte[len];
        System.arraycopy(data, 4 + 4, result, 0, len);
        return result;
    }

    public byte[] encodeProtocol(byte[] data, int len) {
/*
